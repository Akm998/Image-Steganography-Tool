...
class IMG_Stegno:
    ...
    def encode_frame2(self,e_F2):
        ...
        if not myfile:
            ...
            label2.grid(pady=15)
            text_a = Text(e_pg, width=50, height=10)#text_a = our hidden text
            text_a.grid()
            encode_button = Button(e_pg, text='Cancel', command=lambda : IMG_Stegno.back(self,e_pg))
            encode_button.config(font=('Segoe UI',14), bg='#e8c1c7')
    ...
    def decode_frame2(self,d_F2):
        ...
        if not myfiles:
            ...
            label4.config(font=('Segoe UI',14,'bold'))
            label4.grid()
        ...
        while (True):
            ...
            if pixels[-1] % 2 != 0:
                ...
                for i in range(len(data)):
                    ...
                    if (char.isupper()):
                        result += chr((ord(char) - 4-65 +26) % 26 + 65)
                    
                    # Decrypt lowercase characters
                    elif (char.islower()):
                        result += chr((ord(char) - 4-97+26) % 26 + 97)

                    # Decrypt numbers
                    elif (char.isdigit()):
                        result += chr((ord(char) - 4-48+10) % 10 + 48)
    ...
    def enc_fun(self,text_a,myImg):
        ...
        for i in range(len(data)):
                ...
                # Encrypt numbers
                    ...
                    result += chr((ord(char) + 4 - 48) % 10 + 48)
